...
- C언어는 1989년 ANSI C (C89)표준화 이후 1999년 (C99)과 2011년 (C11)에 주요 업데이트를 거쳤습니다.
이 표준들은 새로운 기능을 추가하고 언어의 능력을 확장했습니다.

- C99의 주요 새 기능
    1. 가변 길이 배열 (Variable-Length Arrays, VLA)
        - 함수 내에서 런타임에 크기가 결정되는 배열을 선언할 수 있습니다.

        ```c
        void function(int n) {
            int array[n];  // n의 값에 따라 배열 크기가 결정됨
            // 배열 사용
        }
        ```

    2. 지정 초기화자 (Designated Initializers)
        - 배열이나 구조체의 특정 요소를 지정하여 초기화할 수 있습니다.
    ...
    3. 복합 리터럴(Compound Literals)
        ...
        int main() {
            function((struct Point){.x = 5, .y = 10});
            return 0;
        }
        ```
    ...
    6. `long long int`타입
        ...
        ```c
        long long int big_number = 9223372036854775807LL;
        ```

2. C11의 주요 새 기능
    1. 익명 구조체와 공용체 (Anonymous Structures and Unions)
        - 구조체나 공용체 내에 이름 없는 구조체나 공용체를 포함할 수 있습니다.
    ...
    2. `_Generic` 선택 표현식
        - 컴파일 시간에 타입에 따라 다른 코드를 선택할 수 있습니다.
    ...
    3. 스레드 지원 (`<thread.h>`)
        ...
        int main() {
            thrd_t thread;
            int value = 42;
            thrd_create(&thread, run, &value);
            thrd_join(thread, NULL);
            return 0;
        }
        ```

    4. 원자적 연산 지원 (`<stdatomic.h>`)
        - 원자적 연산을 위한 타입과 함수들이 추가되었습니다.
    ...
    5. `_Static_assert`키워드
        - 컴파일 시간에 조건을 검사할 수 있습니다.

        ```c
        _Static_assert(sizeof(int) == 4, "Int must be 4 bytes");
        ```

    6. 유니버설 문자 이름 (Universal Character Names)
        - 식별자에 유니코드 문자를 사용할 수 있습니다.
...
- 주의사항
    1. 모든 컴파일러가 C99, C11의 모든 기능을 지원하는 것은 아닙니다. 사용 전 컴파일러 지원 여부를 확인해야 합니다.
    2. 가변 길이 배열은 주의해서 사용해야 합니다. 스택 오버플로우의 위험이 있습니다.
    3. 새로운 기능들은 기존 코드의 호환성을 해칠 수 있으므로, 기존 프로젝트에 도입할 때는 주의가 필요합니다.
    4. C11의 일부 기능(예: 스레드 지원)은 선택적 구현사항이므로, 모든 환경에서 사용 가능하지 않을 수 있습니다.